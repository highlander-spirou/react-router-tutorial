...
import { searchProfilesParams } from "./query/params";
...
export const loader =
  ...
  async ({ request }): Promise<any> => {
    ...
    return defer({
      data: queryClient.ensureQueryData(searchProfilesParams(q)),
    });
  };

const AsyncView = ({ q }) => {
  const { data: profiles } = useQuery<ProfileInterface[]>(
    searchProfilesParams(q)
  );
  return (
    <>
    ...
    <>
      ...
      <Suspense fallback={<p>Loading...</p>}>
        <Await resolve={data} errorElement={<ErrorPage />}>
          <AsyncView q={searchParams.get("search")} />
        </Await>
      </Suspense>
    </>
  );
